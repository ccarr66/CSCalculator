    ...
    partial class MainWindow : Form
    {
        ...
        public class matrix
        {
            ...
            public matrix(string id, int RowsM, int colsN)
            {
                ...
                array = new cmplxNumber[RowsM, colsN];
            }

            public cmplxNumber this[int keyR, int keyC]
            {
                get
                {
        ...
        public class cmplxNumber
        {
            ...
            public static implicit operator string(cmplxNumber d)
            {
                if (d.polarForm)
                {
                    return d.A.ToString() + "* ( Cos(" + d.B.ToString() + ") + i*Sin(" + d.B.ToString() + ") )";
                }
                else
                {
                    if (d.B == 0)
                        ...
                        return d.A.ToString() + " - " + (-1 * d.B).ToString() + 'i';
                    else
                        return d.A.ToString() + " + " + d.B.ToString() + 'i';
                }
            }

            public static implicit operator cmplxNumber(double d)
            {
                return new cmplxNumber(d, 0);
            }
        ...
        {
            public List<cmplxNumber> coefficients;

            public polynomial()
            {
                coefficients = new List<cmplxNumber>();
            }


            public polynomial(params cmplxNumber[] listOfCoeff)
            {
                coefficients = new List<cmplxNumber>();
            ...
            public cmplxNumber this[int key]
            {
                get
                {
            ...
            public static implicit operator string(polynomial d)
            {
                string output = "";
                for (int i = d.coefficients.Count - 1; i >= 0; i--)
                {

                    if (i != 0)
                    {
                        output += d.coefficients[i] + "x^" + i;
                        output += " + ";
                    }
                    else
                    {
                        output += d.coefficients[i] + i;
                    }
                }
                return output;
            ...
            public static implicit operator polynomial(cmplxNumber d)
            {
            ...
            public static implicit operator cmplxNumber(polynomial d)
            {
                return d[0];
            }
            ...
            {
                cmplxNumber[] coefficients = new cmplxNumber[first.degree + second.degree + 1];
                for (int i = 0; i < coefficients.Length; i++)
                {
                    coefficients[i] = new cmplxNumber();
                }


                //cmplxNumber[,] coefficientArray = new cmplxNumber[first.coefficients.Count, second.coefficients.Count];
            ...
            public static polynomial operator *(polynomial first, cmplxNumber second)
            {
                for (int i = 0; i <= first.degree; i++)
                {
            ...
            public static polynomial calcDeterminant(polynomial[,] polyMat)
            {
                ...
                {
                    polynomial stepOne = polyMat[0, 0] * polyMat[1, 1];
                    polynomial stepTwo = polyMat[0, 1] * polyMat[1, 0];
                    polynomial stepThree = stepOne - stepTwo;
                    return stepThree;
                }
                else
                {
        ...
        private void btn_SaveMatrix_Click(object sender, EventArgs e)
        {
            ...
            if (cmb_SaveMatrix.SelectedIndex >= 0 && cmb_SaveMatrix.SelectedIndex < cmb_SaveMatrix.Items.Count)
            {
                ...
                if (txt_MatrixName.Text.Length <= 4 && txt_MatrixName.Text.Length > 0 && !cmb_MatrixStorage.Items.Contains(txt_MatrixName.Text))
                {
                    ...
                    matrixStorage.Add(new matrix(txt_MatrixName.Text, currMatrix.colsN, currMatrix.rowsM));
                    for (int k = 0; k < currMatrix.rowsM; k++)
                        for (int j = 0; j < currMatrix.colsN; j++)
                        {
                            double valHolder;
                            if (!double.TryParse(currMatrix.array[k][j].Text, out valHolder))
                                matrixStorage[matrixStorage.Count - 1].array[k, j] = 0;
                            else
                                matrixStorage[matrixStorage.Count - 1].array[k, j] = valHolder;
                        }
                }
                else
                {
        ...
        {
            cmplxNumber[,] newArray = new cmplxNumber[toSwap.matrixDimMN.Height, toSwap.matrixDimMN.Width];
        ...
        public void rowOpMult(ref matrix toSwap, cmplxNumber k, int rowM)
        {
            for (int j = 0; j < toSwap.matrixDimMN.Width; j++)
            {
        ...
        public void rowOpMultPlus(ref matrix toSwap, cmplxNumber k, int rowM, int rowN)
        {
            for (int j = 0; j < toSwap.matrixDimMN.Width; j++)
            {
        ...
        public void createOutputMatrix(int outMatDest, matrix output)
        {
            ...
            for (int i = 0; i < output.matrixDimMN.Height; i++)
            {
                ...
                for (int j = 0; j < output.matrixDimMN.Width; j++)
                {
                    ...
                    matDisplayOutputs[outMatDest].array[i][j].Text = output.array[i, j];
        ...
        private void btn_RREF_Click(object sender, EventArgs e)
        {
            ...
            for (int mat = 0; mat < 3; mat++)
            {
                ...
                {
                    matrix RREFmat = new matrix("RREF" + mat, matDisplays[mat].rowsM, matDisplays[mat].colsN);
                    ...
                    for (int i = 0; i < matDisplays[mat].rowsM; i++)
                    {
                        ...
                        {
                            double valHolder;
                            if (!double.TryParse(matDisplays[mat].array[i][j].Text, out valHolder))
                                RREFmat.array[i, j] = 0;
                            else
                                RREFmat.array[i, j] = valHolder;
                        }
                    }
        ...
        public matrix RREF(matrix RREF)
        {
            ...
            for (int i = 0; i < RREF.matrixDimMN.Height; i++)
            {
                ...
                cmplxNumber rowDivider = 1 / RREF.array[i, workingCol];
                ...
                for (int k = 0; k < RREF.matrixDimMN.Height; k++)
                {
                    if (k != i)
                    {
                        cmplxNumber rowIMult = -RREF.array[k, workingCol];
        ...
        private void btn_Multiply_Click(object sender, EventArgs e)
        {
            ...
            for (int mat = 0; mat < 3; mat++) 
            {
                if (activeDisplays[mat])
                {
                    matricesToMultiply.Add(new matrix("mult", matDisplays[mat].rowsM, matDisplays[mat].colsN));
                    ...
                    for (int i = 0; i < matDisplays[mat].rowsM; i++)
                    {
                        ...
                        {
                            double outVal;
                            if (!double.TryParse(matDisplays[mat].array[i][j].Text, out outVal))
                            {
                                matricesToMultiply[matricesToMultiply.Count - 1].array[i, j] = 0;
                            }
                            else
                            {
                                matricesToMultiply[matricesToMultiply.Count - 1].array[i, j] = outVal;
                            }
        ...
        public matrix matrixMultiplication(matrix multiOne, matrix multiTwo)
        {
            if (multiOne.matrixDimMN.Width == multiTwo.matrixDimMN.Height)
            {
                ...
                matrix product = new matrix("product", multiOne.matrixDimMN.Height, multiTwo.matrixDimMN.Width);
        ...
        private void btn_Transpose_Click(object sender, EventArgs e)
        {
            ...
            for (int mat = 0; mat < 3; mat++)
            {
                if (activeDisplays[mat])
                {
                    matrix transpose = new matrix("transpose", matDisplays[mat].colsN, matDisplays[mat].rowsM);
                    ...
                    for (int i = 0; i < transpose.matrixDimMN.Height; i++)
                    {
                        ...
                        {
                            double outVal;
                            if (!double.TryParse(matDisplays[mat].array[j][i].Text, out outVal))
                            {
                                transpose.array[i, j] = 0;
                            }
                            else
                            {
                                transpose.array[i, j] = outVal;
        ...
        private void btn_Determinant_Click(object sender, EventArgs e)
        {
            ...
            for (int mat = 0; mat < 3; mat++)
            {
                ...
                {
                    matrix determinant = new matrix("determinant", matDisplays[mat].rowsM, matDisplays[mat].colsN);
                    ...
                    for (int i = 0; i < determinant.matrixDimMN.Height; i++)
                    {
                        for (int j = 0; j < determinant.matrixDimMN.Width; j++)
                        {
                            ...
                            {
                                determinant.array[i, j] = outVal;

                            }
                        }
                    }

                    cmplxNumber det = calcDeterminant(determinant);
        ...
        public cmplxNumber calcDeterminant(matrix det)
        {
            ...
            else
            {
                ...
                {
                    matrix minor = new matrix("determinant" + i, det.matrixDimMN.Height - 1, det.matrixDimMN.Height - 1);
        ...
        private void btn_Adjoint_Click(object sender, EventArgs e)
        {
            ...
            for (int mat = 0; mat < 3; mat++)
            {
                if (activeDisplays[mat] && matDisplays[mat].colsN == matDisplays[mat].rowsM)
                {
                    matrix adjoint = new matrix("adjoint" + mat, matDisplays[mat].rowsM, matDisplays[mat].colsN);
                    ...
                    for (int i = 0; i < adjoint.matrixDimMN.Height; i++)
                    {
                        for (int j = 0; j < adjoint.matrixDimMN.Width; j++)
                        {
                            ...
                            {
                                adjoint.array[i, j] = outVal;
                            }
                        }
                    }

                    int sign = 1;
                    cmplxNumber[,] cofactors = new cmplxNumber[adjoint.matrixDimMN.Height, adjoint.matrixDimMN.Width];

                    for (int i = 0; i < adjoint.matrixDimMN.Height; i++)
                    {
                        for (int j = 0; j < adjoint.matrixDimMN.Width; j++)
                        {
                            matrix minor = new matrix("minor" + i + j, adjoint.matrixDimMN.Height - 1, adjoint.matrixDimMN.Width - 1);
        ...
        private void btn_Inverse_Click(object sender, EventArgs e)
        {
            ...
            for (int mat = 0; mat < 3; mat++)
            {
                if (activeDisplays[mat] && matDisplays[mat].colsN == matDisplays[mat].rowsM)
                {
                    matrix determinant = new matrix("determinant" + mat, matDisplays[mat].rowsM, matDisplays[mat].colsN);
                    ...
                    if (det != 0)
                    {
                        ...
                        cmplxNumber detReciprocal = 1 / det;
        ...
        private void btn_EigenVectorValue_Click(object sender, EventArgs e)
        {
            ...
            for (int mat = 0; mat < 3; mat++)
            {
                if (activeDisplays[mat] && matDisplays[mat].colsN == matDisplays[mat].rowsM)
                {
                    ...
                    //txt_TestLinAl.Text = det.ToString();
                    if (det[det.degree] != 1)
                    {
                        cmplxNumber divideOut = det[det.degree];
                    ...
                    int loopCnt = 0;
                    List<cmplxNumber> allRoots = DurandKernerMethod(det, ref loopCnt);

                    //List<cmplxNumber> roots = new List<cmplxNumber>(allRoots);
                    //List<int> multiplicities = new List<int>(allRoots.Count);

                    List<Tuple<cmplxNumber, int>> roots = new List<Tuple<cmplxNumber, int>>(allRoots.Count);

                    for (int r = 0; r < allRoots.Count; r++)
                    {
                        roots.Add(new Tuple<cmplxNumber, int>(allRoots[r], 0));
                    }

                    for (int r = 0; r < roots.Count; r++)
                    {
                        for (int findM = r+1; findM < roots.Count; findM++)
                        {
                            if(roots[findM] == roots[r])
                            {
                                roots[r] = new Tuple<cmplxNumber, int>(roots[r].Item1, roots[r].Item2 + 1);
                                roots.RemoveAt(findM);
                            }
                        }
                    }

                    List<EigenContainer> EigenVs = new List<EigenContainer>(matDisplays[mat].colsN);
                    ...
                    for (int r = 0; r < roots.Count; r++) 
                    {
                        ...
                        for (int i = 0; i < matDisplays[mat].rowsM; i++)
                        {
                            for (int j = 0; j < matDisplays[mat].colsN; j++)
                            {
                                eigenValSub[i, j] = polynomialArray[i, j].evaulate(roots[r].Item1);
                            }
                        }

                        eigenValSub = RREF(eigenValSub);

                        List<int> potentialEVColumns = new List<int>();

                        for (int col = 0; col < eigenValSub.matrixDimMN.Width; col++)
                            potentialEVColumns.Add(col);

                        for (int col = 0; col < eigenValSub.matrixDimMN.Width; col++)
                        {
                            bool pivot = true;
                            for (int row = 0; row < eigenValSub.matrixDimMN.Height; row++)
                            {
                                if(eigenValSub[row,col] != 0 && eigenValSub[row, col] != 1)
                                {
                                    pivot = false;
                                }
                            }
                            if (pivot)
                                potentialEVColumns.Remove(col);
                        }

                        for (int p = 0; p < potentialEVColumns.Count; p++)
                        {
                            matrix eigenvector = new matrix("eigenvector", eigenValSub.matrixDimMN.Height, 1);
                            for (int row = 0; row < eigenValSub.matrixDimMN.Height; row++)
                            {
                                eigenvector[row, 0] = -1*eigenValSub[row, potentialEVColumns[p]];
                            }
                            EigenContainer newEV;
                            newEV.eigenValue = roots[r].Item1;
                            newEV.eigenVector = eigenvector;
                    ...
                    matrix outputEVs = new matrix("outputEVs", matDisplays[mat].rowsM + 1, matDisplays[mat].colsN);
                    for (int row = 0; row < outputEVs.matrixDimMN.Height; row++)
                    {
                        for (int col = 0; col < outputEVs.matrixDimMN.Width; col++)
                        {
                            outputEVs[row, col] = 0;
                        }
                    }

                    for (int ev = 0; ev < EigenVs.Count; ev++)
                    {
                        outputEVs[0, ev] = EigenVs[ev].eigenValue;
                        for (int row = 1; row < outputEVs.matrixDimMN.Height; row++)
                        {
                            outputEVs[row, ev] = EigenVs[ev].eigenVector[row - 1, 0];
                        }
                    }

                    createOutputMatrix(mat, outputEVs);
